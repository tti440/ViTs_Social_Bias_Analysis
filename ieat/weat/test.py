...
class Test:
    ...
    def effect_size(self):
        ...
        """
        self.xab = np.mean(self.s_wAB(np.arange(self.X.shape[0])))
        self.yab = np.mean(self.s_wAB(np.arange(self.X.shape[0], self.similarity_matrix.shape[0])))
        numerator = self.xab - self.yab
