from ieat.models import SENTExtractor, OpenAIExtractor, LogitExtractor, ViTExtractor, DinoExtractor, SWINExtractor
...
def test_all(
	...
	for model_type, model_params in model_types.items():
		...
		for test_data in to_test:
			...
			results[(test_data.name, model_type)] = (*categories, effect, xab, yab, p, n_target, n_attr)
...
def _load_model(model_type, *model_params, **model_kwargs):
	if model_type == "igpt-logit":
	...
	elif model_type == "swin" or model_type=="simmm":
		assert len(model_params) == 2, "The parameter only can contain checkpoint and boolean for backbone" # model[0] - checkpoint, model[1] - backbone or not
	...
	elif model_type == "beit" or model_type == "moco" or model_type == "mae" or \
			model_type == "vit" or model_type == "maskfeat" or model_type == "milan" or model_type == "deit":
		assert len(model_params) == 2, "The parameter only can contain checkpoint and boolean for backbone" # model[0] - checkpoint, model[1] - backbone or not
		return ViTExtractor(
			model_type,
			*model_params,
		...
		assert len(model_params) == 3, "The parameter only can contain checkpoint, name, boolean for backbone" 
